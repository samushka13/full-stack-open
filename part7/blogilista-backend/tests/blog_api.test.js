...
describe("when there are initial blogs", () => {
  before(async () => {
    ...
    TOKEN = jwt.sign(userForToken, process.env.SECRET, { expiresIn: 3600 });

    TEST_DATA = helper.TEST_BLOGS.map((b) => ({
      ...b,
  ...
  test('succeeds with 200 if all blogs include "id"', async () => {
    ...
    const ids = response.body.flatMap((r) =>
      Object.keys(r).filter((k) => k === "id"),
    );
